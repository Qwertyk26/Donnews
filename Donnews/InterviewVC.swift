...
class InterviewVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(InterviewTVC.self, forCellReuseIdentifier: cellId)
        refreshControl.addTarget(self, action: #selector(refresh), for: UIControlEvents.valueChanged)
        tableView.addSubview(refreshControl)
        fetchResult()
    }
    func fetchResult() {
        Alamofire.request("http://www.donnews.ru/json/api.getArticleList?offset=0&count=10&category=interview").responseJSON { response in
            ...
            case .success(let JSON):
                ...
                for resultItems in result {
                    ...
                    let newsItem = NewsEntity(alias: alias!, category: .InterView, title: title!, text: text!, lead: lead!, image_full: image_full!, image_width: image_width!, image_height: image_height!, category_name: category_name!, comments_count: comments_count!, date: date!, post: post!, expert_name: expert_name!, expert_post: expert_post!, author_name: "")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath) as! InterviewTVC
    ...
    func loadMore(offset: Int) {
        Alamofire.request(String.init(format: "http://www.donnews.ru/json/api.getArticleList?offset=%d&count=10&category=interview", offset)).responseJSON { response in
            switch(response.result) {
            case .success(let JSON):
                let response = JSON as! NSDictionary
                let result = response.object(forKey: "result") as! [AnyObject]
