...
class SpecprojectsVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(SpecprojectsTVC.self, forCellReuseIdentifier: cellId)
        refreshControl.addTarget(self, action: #selector(refresh), for: UIControlEvents.valueChanged)
        tableView.addSubview(refreshControl)
        fetchResult()
    }
    func fetchResult() {
        Alamofire.request("http://www.donnews.ru/json/api.getArticleList?offset=0&count=10&category=specproects").responseJSON { response in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath) as! SpecprojectsTVC
    ...
    func loadMore(offset: Int) {
        Alamofire.request(String.init(format: "http://www.donnews.ru/json/api.getArticleList?offset=%d&count=10&category=specproects", offset)).responseJSON { response in
            switch(response.result) {
            case .success(let JSON):
