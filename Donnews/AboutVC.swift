...
class AboutVC: UIViewController, UIWebViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("http://www.donnews.ru/json/api.About").responseJSON { response in
            switch(response.result) {
            case .success(let JSON):
                self.activityIndicator.stopAnimating()
                let response = JSON as! NSDictionary
                let result = response.object(forKey: "result") as AnyObject
                self.webView.loadHTMLString(String.init(format: "<html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><link href=\"about.css\" rel=\"stylesheet\" type=\"text/css\" /></head><body>%@</body></html>", result.value(forKey: "about") as! String), baseURL: path)
                break
